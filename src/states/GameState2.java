...
public class GameState2 extends GameState{
    ...
    //private int lives=super.getLives();
    ...
    //Musica
    private Sound backgroundMusic, backgroundMusic2;
    ...
    //Laberinto de Basura
    //private LaberynthState labSta=new LaberynthState();
    ...
    public GameState2() {
       ...
       // startWave();
        ...
        backgroundMusic2 = new Sound(Assets.musicaJuego);
        backgroundMusic2.loop();
        backgroundMusic2.changeVolume(2);
        ...
        ojoSpawner.run(Constants.OJO_SPAWN);
         ...
         tiempoFin.run(Constants.CAMBIOFASE2);
   ...
   public void addScore(int valor, Vector2D position){
    ...
    public void playExplosionSangre(Vector2D position){
        explosionSangre.add(new Animation(50, position.subtract(new Vector2D(Assets.explosionSangre[0].getWidth()/2,
                                Assets.explosion[0].getHeight()/2)),Assets.explosionSangre));
    }
 public void choqueLaseres(Vector2D position){
   ...
   @Override
    public void update(){
       ...
       gameOverTimer.update();
        ...
        if(!tiempoFin.isRunning()&&!gameOver){
           // ultimoMensaje.ultimaOleada();
           backgroundMusic2.stop();
            state.changeState(new Ready3());
        }
        /*...............................*/ 
    }
    ...
    /*EN ESTE MÉTODO EL ORDEN ES DE SUMA IMPORTANCIA, si está arriba las cosas se colocan
       ...
       for (int i = 0; i < messages.size(); i++) {
            ...
            if(messages.get(i).finTexto())messages.remove(i);
        }
       drawScore(g);
       drawLives(g);
       drawTiempo(g);
                          ...
                          for (int j = 0; j < laberintoBasura[i].length; j++) {
                              switch(i){
                                  case 0:
                                      ...
                                      laberintoBasura[i][j]=new Laberynth(new Vector2D(XLa+consX, YLa), Assets.trashAll[tipodeBasura-1], GameState2.this, velocity);
                                  ...
                                  case 1: 
                                       tipodeBasura=(int)(Math.random()*(1-4)+4);
                                      consY+=110;
                                          laberintoBasura[i][j]=new Laberynth(new Vector2D(XLa+consX, YLa+consY), Assets.trashAll[tipodeBasura-1], GameState2.this, velocity);
                                          movingObjects.add(laberintoBasura[i][j]);
                                      /*if(j>0){
    ...
    public void gameOverMetodo(){
        ...
        gameOver=true;
        if(gameOver)backgroundMusic2.stop();
  ...
  /* public  int[][] getLaberynth(){
    ...
    /*................OJO................*/
    private void spawnOJO(){
       
        //Posición inicial del Ojo
            double x,y;
            x=Math.random()*(100-1000)+1000;
            y=-100;
        //Array que representa los nodos del camino
        ArrayList<Vector2D>nodos=new ArrayList<Vector2D>();
        // Posición de cada nodo para la ruta del OJO
        double positionX, positionY;
        /*A continuación vamos a dividir la pantalla en 4 extremos superior
        izquierdo, superior derecho, inferior izquierdo e inferior derecho e iremos
        metiendo los valores en la array, para que se mueva de izquierda a derecha*/
        //PRIMER NODO
        	positionX = Math.random()*Constants.WIDTH/2;
		positionY = Math.random()*Constants.HEIGHT/2;	
		nodos.add(new Vector2D(positionX, positionY));
        //SEGUNDO NODO        
		positionX = Math.random()*(Constants.WIDTH/2) + Constants.WIDTH/2;
		positionY = Math.random()*Constants.HEIGHT/2;	
		nodos.add(new Vector2D(positionX, positionY));
	//TERCER NODO	
		positionX = Math.random()*Constants.WIDTH/2;
		positionY = Math.random()*(Constants.HEIGHT/2) + Constants.HEIGHT/2;	
		nodos.add(new Vector2D(positionX, positionY));
	//CUARTO NODO	
		positionX = Math.random()*(Constants.WIDTH/2) + Constants.WIDTH/2;
		positionY = Math.random()*(Constants.HEIGHT/2) + Constants.HEIGHT/2;	
		nodos.add(new Vector2D(positionX, positionY));
               ...
               /*Lo metemos en nuestra array de objetos que se mueven por pantalla*/
        	movingObjects.add(new OJO( new Vector2D(x, y), Assets.ojo, this, Constants.OJO_MAX_VEL,
                                nodos,new Vector2D()));
